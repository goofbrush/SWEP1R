...
// public:
    ...
    //         if (timer > t && f == false) {
    //             r.setPlayerBlock(current->playerBlock);
    //             t = timer;
    //             current++;
    //             // f = true;
    //         }
    ...
    //     }
    //     else if(raceFlag == true) {
    //         if(recMode) { r.rec.saveRecording(); }
    ...
    // void displayValue(BYTE* block, int pos, QLabel* l) {
    //     Type value = *(Type*)(block + pos);
    //     // std::string hex = intToHexString(pos) + ": " + std::to_string(value);
    //     // l->setText(hex);
    // }
